...
**Export `register` and `save` from `editor.tsx` directly**, rather than testing them indirectly through the real `@wordpress/hooks` filter registry (`applyFilters('blocks.registerBlockType', ...)`). Alternative considered: register the filters for real in a test and invoke via `applyFilters`. Rejected as unnecessary indirection for what are just two pure(ish) functions - direct export is simpler and doesn't require importing/initializing the hooks registry in tests.
